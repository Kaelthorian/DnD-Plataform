...
- La coleccion de combate usa una fotografia revisionada y memoizacion por pasada. Los cambios de campos/meta deben llamar `invalidateCombatActionCache(...)`; las mutaciones directas de `__sheetMeta.equippedItems` pasan por `notifyEquipmentCombatStateChanged()` para refrescar o recalentar Start Combat. No reintroducir escaneos de `items` ni validaciones que llamen `combatTurnState()` por cada tarjeta.
