...
| Runtime/UI de items | `src/data/items/items.json`, `src/engine/items/item-catalog.js`, `item-resource-state.js` | inventario/recursos en `index.html`, índice en `renderer.js`, biblioteca React; sólo `item` es catálogo activo superior |
| Cache/carga de items | `src/services/data-loader.js`, `src/services/workers/item-data-worker.js` | IPC `items:load`; cache compilada v2 en `userData/data-cache/`; indice en memoria en `renderer.js` |
...
- Equipment mantiene el campo PDF como almacenamiento, pero `createEquipmentPanel()` pinta una lista interactiva transparente sobre el fondo de la hoja; conserva controles oscuros y acento ámbar sin cubrir la superficie PDF. Los botones de Attacks and Spellcasting reutilizan ese mismo tratamiento oscuro/ámbar y permiten envolver etiquetas largas en varias líneas; sus indicadores de daño son círculos SVG de tamaño fijo con iconos minimalistas por tipo. El `+` abre el picker de dos paneles `item-picker`: resultados/filtros a la izquierda y `buildItemDrawerContent()` reutilizado como detalle a la derecha, con cantidad y `Sumar item` en el mismo modal. Agregar, quitar, equipar, cambiar agarre y consumir pasan por `scheduleEquipmentMutationRefresh()`; no emitir `input`+`change` sintéticos ni reconstruir paneles sincrónicamente. `globalThis.dndEquipmentPerformance` permite inspeccionar cola/duración. No volver a abrir `itemDrawer` al seleccionar una fila del picker ni crear otro catálogo de items.
- `src/engine/items/item-resource-state.js` normaliza el tracker de cargas/reload numéricas y casting times; `index.html` lo persiste en `__sheetMeta.itemResources[catalogId]`, muestra ajuste `-1/+1` acotado y crea acciones sólo para usos deterministas. Costos numéricos de `attachedSpells.charges` se descuentan después del commit. CA persistente segura, saves/spell bonuses, defensas visibles, consumibles, munición compatible, attunement determinista y efectos guiados comparten los adaptadores de Equipment. Recargas con dados, usos `daily`/`rest`/`will`, requisitos narrativos y reglas contextuales siguen manuales. Dos copias del mismo `catalogId` comparten tracker hasta que Equipment tenga identidad de instancia.
...
- Carga: `data-loader.js` y su worker leen `src/data/items/items.json` más `items-base.json`, invalidan la caché binaria v2 al cambiar cualquiera y entregan el resultado por `items:load`.
- Consumo: `item-catalog.js` comparte identidad, tags y perfiles; `item-resource-state.js` mantiene transiciones puras y acotadas de cargas/reload. `renderer.js` construye el índice del Character Sheet; `index.html` controla inventario, equipo, `itemResources`, `itemAttunement`, `itemEffects` y combate; `dm-screen/src/main.jsx` usa la misma colección activa para búsqueda/detalle.
