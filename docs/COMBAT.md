...
- La animación publica posiciones intermedias por el mismo dm:vtt:patch del drag existente, por lo que DM y jugadores ven el deslizamiento. El snapshot de combate también publica movementTokenKey, movementSpeedFeet, movementUsedFeet y movementRemainingFeet; cuando el participante activo es el personaje del jugador, Start Combat actualiza su contador con ese presupuesto. Una forma adjunta acompaña a la ficha.
...
3. El stepper solicita sólo los pasos declarados: target, attack roll, save/check, damage/healing, efecto y confirmación. El target es un selector: conserva siempre el propio personaje como opción mínima; con combate VTT activo añade exactamente la misma lista visible de combat.participants que el tracker superior, sin añadir tokens del mapa fuera de combate. No acepta texto libre ni pide AC; los resultados Hit/Miss no determinados por natural 1/20 se confirman con el DM.
