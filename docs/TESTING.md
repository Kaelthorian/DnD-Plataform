...
| Catálogo de items | `npm run test:items` | 1.779 activos, identidad nombre+fuente+variante, campos/reglas representativos, preview/apply/check/restore e idempotencia |
...
`scripts/validate-items.js` comprueba el catálogo versionado sin depender de Downloads: exige exactamente 1.779 elementos activos, `catalogId`/`catalogKey` únicos y reproducibles, fuentes/nombres válidos, variantes identificadas y tombstones fuera de la colección activa. `tests/engine/item-data.test.js` recorre armas, armaduras, cargas, attunement, spells, consumibles, venenos, variantes, referencias estructuradas, metadata heredada de tipos, CA held/worn segura, bonos persistentes y preferencia moderna del fallback legacy. `tests/engine/item-resource-state.test.js` fija inicialización idempotente, migración de máximo, límites y detección conservadora de acciones/costos/casting time. `tests/renderer/item-catalog-integration.test.js` y `item-resource-integration.test.js` protegen identidad e historia, búsqueda de variantes, ownership/attunement, CA/saves/spell bonuses, defensas, accesorios, munición/packs, consumibles, efectos guiados, timing individual, rechazo de spellbooks falsamente castables y descuento de cargas sólo después del commit. Son contratos estáticos de wiring con lectura del source/regex; complementan, pero no sustituyen, un smoke DOM/Electron. `tests/services/item-sync.test.js` usa directorios temporales para fijar preview, reemplazo/alta/baja, backup portable, restore sobre targets actuales, relocación, corrupción/rechazo, compatibilidad de manifiestos v1 y el no-op de una segunda ejecución; no toca el catálogo ni los datos de usuario reales.
