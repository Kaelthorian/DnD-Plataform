...
assert.match(source, /const expectedCount = Number\.isInteger\(declaredExpectedCount\) && declaredExpectedCount > 0 \? declaredExpectedCount : 2253/);
...
assert.equal(topLevelItems.length, 2253, "DM picker source must stay at exactly 2,253 top-level rows");
assert.equal(specificVariants.length, 2643, "DM child selector must expose every specific item variant");
