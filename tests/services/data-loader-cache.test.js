...
async function run() {
  ...
  const automationPath = path.join(root, "item-automation.json");
  ...
  try {
    await Promise.all([
      ...
      fs.writeFile(automationPath, JSON.stringify({ schemaVersion: 1, items: [] }))
    ]);

    dataLoader.clearItemCatalogMemoryCache();
    const first = await dataLoader.loadItems(root, { itemsPath, baseItemsPath, automationPath, cachePath });
    ...
    dataLoader.clearItemCatalogMemoryCache();
    const second = await dataLoader.loadItems(root, { itemsPath, baseItemsPath, automationPath, cachePath });
    ...
    dataLoader.clearItemCatalogMemoryCache();
    const refreshed = await dataLoader.loadItems(root, { itemsPath, baseItemsPath, automationPath, cachePath });
    ...
    await new Promise((resolve) => setTimeout(resolve, 20));
    await fs.writeFile(automationPath, JSON.stringify({ schemaVersion: 1, items: [{ match: { name: "Updated Item", source: "TST" }, resources: [], capabilities: [] }] }));
    dataLoader.clearItemCatalogMemoryCache();
    const overlayRefreshed = await dataLoader.loadItems(root, { itemsPath, baseItemsPath, automationPath, cachePath });
    assert.strictEqual(overlayRefreshed.cacheMeta.source, "source-json", "overlay changes should invalidate the persistent cache");
    assert.strictEqual(overlayRefreshed.automation.items.length, 1);
  } finally {
    dataLoader.clearItemCatalogMemoryCache();
