...
- Las capabilities `oneTime` se gastan sólo al confirmar y se persisten por `catalogId::capability:<id>` en `__sheetMeta.itemCapabilityUses`; no modelar su consumo como un recurso que reaparece con un descanso. `activatesStatus` debe apuntar a un status existente y removible del motor de condiciones.
