...
- La selección de targets de combate reutiliza `dndLiveVttCombatTargetRoster()` y los participantes visibles del tracker: el ID opaco viaja en la opción y la sesión, mientras el nombre visible se usa para UI/log. Los snapshots/patches VTT deben refrescar una resolución abierta sin crear otro store.
- La escala VTT canónica es el token default de 56 unidades = 5 ft. En combate el DM mueve centro a centro mediante selección + click, y `src/engine/combat/vtt-movement.js` limita el recorrido por el Speed de la nota; no dupliques esta conversión en los renderers.
...
- Disponibilidad de spells: no añadir una prohibición global de combate. `src/engine/combat/action-definitions.js` deriva `requiresCombat` desde metadata/evidencia de la acción, y un spell no combativo con target puede reutilizar el stepper fuera de combate con coste de turno cero.
